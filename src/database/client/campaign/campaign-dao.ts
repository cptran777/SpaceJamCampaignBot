...
export class CampaignDAO {
  ...
  private dbSchemaToTsCampaign(databaseObject: IDBSchemaCampaign): ICampaign {
    const { name, owner_id, admin_ids, member_ids, characters, character_ownership, currency, money } = databaseObject;
  ...
  private tsCampaignToDBSchema(campaignObject: ICampaign): IDBSchemaCampaign {
    const { name, ownerID, adminIDs, memberIDs, characters, characterOwnership, currency, money } = campaignObject;
  ...
  async getCampaignByName(name: string): Promise<ICampaign | null> {
    ...
    if (getCampaignResponse.rows.length === 0) return null;
  ...
  async saveCampaign(campaign: ICampaign): Promise<void> {
    ...
    await this.client.query(
      ...
      `,
      [name, owner_id, admin_ids, member_ids, characters, character_ownership, currency, money]
    );
  }
}